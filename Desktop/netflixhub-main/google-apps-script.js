...
// Admin Configuration
const ADMIN_PASSWORD = '12Oktober2019'; // Ganti dengan password Anda
    ...
    try {
        ...
        if (action === 'addSubscription') {
            const data = {
                ...
                // NEW: Sharing profile fields
                profileType: e.parameter.profileType || 'private',
                slotNumber: e.parameter.slotNumber || '',
                isSlotOwner: e.parameter.isSlotOwner || 'true',
                price: e.parameter.price || 50000
            };
 ...
 * Netflix OTP bisa 6 digit (verifikasi) atau 4 digit (kode masuk/login code)
 */
function extractOTP(subject, body) {
    const subjectLower = subject.toLowerCase();

    // ===== KODE MASUK 4 DIGIT =====
    // Email "Netflix: Kode masukmu" atau "Netflix: Your login code"
    // Berisi kode 4 digit untuk login
    if (subjectLower.includes('kode masuk') ||
        subjectLower.includes('login code') ||
        subjectLower.includes('sign-in code') ||
        subjectLower.includes('signin code')) {

        // Pattern untuk kode 4 digit
        const fourDigitPatterns = [
            /\b(\d{4})\b/,  // 4 digit berurutan
            /code[:\s]+(\d{4})/i,
            /kode[:\s]+(\d{4})/i
        ];
    ...
    // ===== KODE OTP 6 DIGIT =====
    // Pattern untuk mencari OTP 6 digit di subject atau body
    ...
    // Cek subject untuk email verifikasi
 ...
 * UPDATED: Now includes sharing profile columns
 */
function getSubscriptionsSheet() {
    ...
    if (!sheet) {
        ...
        // UPDATED: Added ProfileType, SlotNumber, IsSlotOwner, Price columns
        sheet.getRange(1, 1, 1, 10).setValues([[
            'ID', 'Email', 'ProfileName', 'PIN', 'DueDate', 'Notes',
            'ProfileType', 'SlotNumber', 'IsSlotOwner', 'Price'
        ]]);
 ...
 * UPDATED: Now returns sharing profile fields
 */
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (data[i][1]) {
                ...
                subscriptions.push({
                    ...
                    notes: data[i][5] || '',
                    // NEW: Sharing profile fields
                    profileType: data[i][6] || 'private',
                    slotNumber: data[i][7] || '',
                    isSlotOwner: data[i][8] !== false && data[i][8] !== 'false' ? true : false,
                    price: data[i][9] || 50000
                });
            }
        }
 ...
 * UPDATED: Now includes sharing profile fields
 */
function addSubscription(data) {
    try {
        ...
        sheet.appendRow([
            ...
            data.notes || '',
            // NEW: Sharing profile fields
            data.profileType || 'private',
            data.slotNumber || '',
            data.isSlotOwner || 'true',
            data.price || 50000
        ]);
        return { success: true };
    } catch (error) {
    ...
    try {
        ...
        for (let i = 1; i < allData.length; i++) {
            if (allData[i][0] === data.id) {
                // UPDATED: Now updates all 9 columns (2-10)
...
/**
 * MIGRATION: Run this once to add new columns to existing sheet
 * Only run if you already have data in your sheet!
 */
function migrateAddSharingColumns() {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(SUBSCRIPTIONS_SHEET_NAME);
    ...
    if (headers.includes('ProfileType')) {
        Logger.log('Columns already migrated');
        return;
    }

    // Add new headers
    const lastCol = sheet.getLastColumn();
    sheet.getRange(1, lastCol + 1, 1, 4).setValues([['ProfileType', 'SlotNumber', 'IsSlotOwner', 'Price']]);
    sheet.getRange(1, lastCol + 1, 1, 4).setFontWeight('bold');
    ...
    if (lastRow > 1) {
        ...
        for (let i = 2; i <= lastRow; i++) {
            defaultValues.push(['private', '', 'true', 50000]);
        }
        sheet.getRange(2, lastCol + 1, lastRow - 1, 4).setValues(defaultValues);
    }

    Logger.log('Migration complete! New columns added: ProfileType, SlotNumber, IsSlotOwner, Price');
}
